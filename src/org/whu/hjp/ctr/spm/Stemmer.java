...
/**
 * * Stemmer, implementing the Porter Stemming Algorithm * * The Stemmer class
 * transforms a word into its root form. The input * word can be provided a
 * character at time (by calling add()), or at once * by calling one of the
 * various stem(something) methods.
 * */
...
class Stemmer {
	 ...
	 * a reference to the internal buffer can be retrieved by getResultBuffer *
	 ...
	 * of files, stems each word, and writes the result to standard * output.
	 * Note that the word stemmed is expected to be in lower case: * forcing
	 * lower case must be done outside the Stemmer class. * Usage: Stemmer
	 * file-name file-name ...
	 * */
